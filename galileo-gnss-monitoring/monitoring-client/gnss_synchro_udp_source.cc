    ...
    if (read_gnss_synchro(stocks))
        {
            ...
            // Print table header.
            attron(A_REVERSE);
            printw("%3s%6s%14s%17s%21s%25s%32s%32s\n", "CH", "PRN", "CN0 [dB-Hz]", "Doppler [Hz]", "Code Phase", "rx_time","TOW_at_current_symbol_ms","Pseudorange");
            attroff(A_REVERSE);
            ...
            for (auto const& ch : channels)
                {
                    ...
                    printw("%3d%6d%14f%17f%21f%25f%32lu%32f\n", channel_id, data.prn(), data.cn0_db_hz(), data.carrier_doppler_hz(), data.acq_delay_samples(), data.rx_time(), data.tow_at_current_symbol_ms(), data.pseudorange_m());

                    if (data.nav_symbol() > 0)
                    { 
                        // bit = 1;
                        main_bit = 1;
